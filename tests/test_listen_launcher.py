...
@pytest.mark.asyncio
async def test_listen_is_the_home_page(client: AsyncClient):
    response = await client.get("/")

    assert response.status_code == 200
    assert "Écouter" in response.text
    assert 'value="papa"' in response.text
    assert "Aucune enceinte Music Assistant" in response.text


@pytest.mark.asyncio
async def test_listen_filters_media_by_owner(client: AsyncClient, db: AsyncSession):
    ...
    response = await client.get("/?owner=papa")
